 ...
 * warning but never wins — data cannot lie about its own day.
 */
export function resolvePositionDate(
  ...
  if (sniffed.date) {
    ...
    return {
      positionDate: sniffed.date,
      dateSource: `derived from the export's own rows (${sniffed.agree}/${sniffed.total} rows)`,
      warnings,
    };
  }
  if (provided) {
    return {
      positionDate: provided,
      dateSource: 'trader-provided (no date derivable from the export rows)',
      warnings,
    };
  }
  throw new Error(
